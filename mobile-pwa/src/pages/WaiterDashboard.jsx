...
const WaiterDashboard = () => {
    ...
    useEffect(() => {
        const user = JSON.parse(localStorage.getItem('user'));
        if (user?.role === 'Mutfak') {
            navigate('/kitchen-dashboard'); // Eğer mutfakçıysa zorla mutfağa geri at
        }
    }, []);
        ...
        <div className="dashboard-layout">
            ...
            <main className="dashboard-content">
                <div className="table-grid">
                    ...
                    {/* DEV HIZLI SATIŞ BUTONU */}
                    <div
                        className="table-card fast-sale-card"
                        onClick={() => navigate('/waiter-order/fast')}
                        style={{
                    ...
                    >
                        <div className="table-number" style={{ color: 'white' }}>⚡ HIZLI SATIŞ</div>
                        <div className="table-status" style={{ color: '#ffeaa7' }}>Gel-Al Sipariş Gir</div>
                    </div>

                    {/* Aşağısı mevcut masa listeleme (tables.map) kodu olarak devam edecek... */}
                    {tables.map(table => (
                        <div
                            key={table.id}
